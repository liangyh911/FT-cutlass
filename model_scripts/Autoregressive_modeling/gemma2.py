import os
# Disable torch compile if it causes issues with specific hardware/versions
...
with open(logFP, "a") as file:
    file.write(f"Start Job (Gemma2 Batch): {faulty_step}\n")
    
controlFP = f"./control_{job_id}/0/perform.txt"
cutlassFP = f"./control_{job_id}/0/cutlass.txt"
SMChkFP = f"./control_{job_id}/0/split.txt"

# --- [Change 1] Model ID for Gemma 2 2B ---
MODEL_ID = "google/gemma-2-2b-it" 
TEMP_OUTPUT_DIR = "./tmp_gemma2_batch_logs"

# Training Config
NUM_TRAIN_SAMPLES = 4000 
SEED = 42
BATCH_SIZE = 8  # Gemma 2 2B is slightly larger than 1B, adjust batch size if OOM
GRAD_ACCUMULATION = 2
LEARNING_RATE = 1e-5 
MAX_SEQ_LENGTH = 1024 
MAX_STEPS = 1000 
...
def format_general_prompt(example):
    ...
    # Standard Chat Format for Gemma 2 (AutoTokenizer handles the specific <start_of_turn> tags)
    messages = [
        {"role": "user", "content": user_content},
        {"role": "model", "content": output} 
        # Note: Gemma uses 'model' instead of 'assistant' in some templates, 
        # but AutoTokenizer usually maps 'assistant' to 'model' correctly.
    ]
    return messages
...
def evaluate_mmlu_batch(model, tokenizer, num_samples=500):
    ...
    #     ds_list = [load_dataset("cais/mmlu", s, split="test") for s in selected]
    ...
    for i in tqdm(range(0, len(subset), batch_size), desc="MMLU Batch"):
        ...
        for j in range(len(batch_items['question'])):
            ...
            # Use simple user/model structure for Gemma 2
            messages = [
                {"role": "user", "content": f"Answer the multiple choice question. Output ONLY the letter (A, B, C, or D).\n\nQuestion: {q}\n\nOptions:\n{opts}\n\nAnswer:"}
            ]
...
def evaluate_squad_batch(model, tokenizer, num_samples=500):
    ...
    for i in tqdm(range(0, len(subset), batch_size), desc="SQuAD Batch"):
        ...
        for j in range(len(batch_items['context'])):
            ...
            messages = [
                {"role": "user", "content": f"Read the context and answer the question. Keep it concise.\n\nContext: {ctx}\n\nQuestion: {qn}\n\nAnswer:"}
            ]
            batch_prompts.append(tokenizer.apply_chat_template(messages, tokenize=False, add_generation_prompt=True))
...
def evaluate_wmt16_batch(model, tokenizer, num_samples=500):
    ...
    for i in tqdm(range(0, len(subset), batch_size), desc="WMT Batch"):
        ...
        for trans in batch_items['translation']:
            ...
            messages = [
                {"role": "user", "content": f"Translate the following German text into English.\n\nGerman: {src}\n\nEnglish Translation:"}
            ]
            batch_prompts.append(tokenizer.apply_chat_template(messages, tokenize=False, add_generation_prompt=True))
...
def evaluate_gsm8k_batch(model, tokenizer, num_samples=500):
    ...
    for i in tqdm(range(0, len(subset), batch_size), desc="GSM8K Batch"):
        ...
        for j in range(len(batch_items['question'])):
            ...
            messages = [
                {"role": "user", "content": f"Solve step by step. End response with 'The answer is #### <number>'.\n\nQuestion: {qn}"}
            ]
            batch_prompts.append(tokenizer.apply_chat_template(messages, tokenize=False, add_generation_prompt=True))
...
def evaluate_xlsum_batch(model, tokenizer, num_samples=500):
    ...
    for i in tqdm(range(0, len(subset), batch_size), desc="XLSum Batch"):
        ...
        for article in batch_items['text']:
            messages = [
                {"role": "user", "content": f"Summarize the news below into a single sentence.\n\nArticle: {article}\n\nSummary:"}
            ]
            batch_prompts.append(tokenizer.apply_chat_template(messages, tokenize=False, add_generation_prompt=True))
...
def main():
    ...
    # [CRITICAL] Left Padding for Batch Gen
    ...
    # [CRITICAL] Switch to Left Padding
    tokenizer.padding_side = "left"
